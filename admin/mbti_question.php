...
/*BEGIN*/
    ...
    case 'verifyQuestionNo':
        try {
            $strsql = "select * from mbti_question where questionNo='" . $_POST['questionNo'] . "'";
    ...
    case 'insert':
        try {
            ...
            $value['choiceA'] = $_POST['choiceA'];
            $value['choiceB'] = $_POST['choiceB'];

            $request = json_decode(json_encode($value), false);
            $result = $db->create('mbti_question', array_keys($value), $request, 'questionID');
    ...
    case 'update':
        try {
            ...
            $value['choiceB'] = $_POST['choiceB'];
            $request = json_decode(json_encode($value), false);
            $result = $db->update('mbti_question', array_keys($value), $request, 'questionID');
            if (isset($result['errors'])) {
                throw new Exception($result['errors'][0]);
            } else {
                echo 1;
    ...
    case 'delete':
        try {
            $value['questionID'] = $_POST["questionID"];
            $request = json_decode(json_encode($value), false);
            $result = $db->destroy('mbti_question', $request, 'questionID');
            if (isset($result['errors'])) {
                throw new Exception($result['errors'][0]);
            } else {
                echo 1;
...
<body>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div
                    ...
                    <h1 class="h2">
                        <?=$_SESSION['setting']['lang'] == 'en' ? 'Personality test question information' : 'ข้อมูลคำถามทดสอบบุคคลิกภาพ';?>
                    </h1>
                </div>
                ...
                <div class="card mt-4">
                    <div class="card-body">
                        <form class="row g-3" id="scriptForm">
                            ...
                            <div class="col-md-3">
                                <label for="choiceA"
                                    class="form-label"><?=$_SESSION['setting']['lang'] == 'en' ? 'Choice A' : 'ตัวเลือก ก';?></label>
                                <div class="col">
                                    <input type="text" class="k-textbox" id="choiceA" name="choiceA"
                                        style="width:100%">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <label for="choiceB"
                                    class="form-label"><?=$_SESSION['setting']['lang'] == 'en' ? 'Choice B' : 'ตัวเลือก ข';?></label>
                                <div class="col">
                                    <input type="text" class="k-textbox" id="choiceB" name="choiceB"
                                        style="width:100%">
                                </div>
                            </div>
    ...
    function page() {
        ...
        page.prototype.initial = function() {
            $('#data-grid').kendoGrid({
                ...
                columns: [
                    ...
                    {
                        field: 'choiceA',
                        title: '<?=$_SESSION['setting']['lang'] == 'en' ? "Choice A" : "ตัวเลือก ก"?>',
                        id: "choiceA"
                    },
                    {
                        field: 'choiceB',
                        title: '<?=$_SESSION['setting']['lang'] == 'en' ? "Choice B" : "ตัวเลือก ข"?>',
                        id: "choiceB"
                    },
        ...
        page.prototype.bindData = function(selectedItem) {
            ...
            $('#choiceA').val(selectedItem.choiceA);
            $('#choiceB').val(selectedItem.choiceB);
        }
